...
class CreateTest extends Migrator
{
    /**
     * Change Method.
     *
    ...
    public function up(){
        ...
        $table = $this->table('test', ['comment' => '测试表', 'engine' => 'InnoDB', '']);
        $table->addColumn('type', 'integer', ['comment' => '类型'])
            ->addColumn('content', 'text', ['comment' => '内容',])
            ->addIndex(['type'], ['unique' => true])//为user_name创建索引并设置唯一(唯一索引)
